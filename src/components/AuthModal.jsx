...
export default function AuthModal({ onClose, defaultTab = 'signin' }) {
  ...
  const [done, setDone]         = useState(false); // post-signup confirmation state

  function switchTab(t) {
    setTab(t); setError(''); setEmail(''); setPassword(''); setAlias(''); setDone(false);
  }
  ...
  async function handleSubmit(e) {
    ...
    // If Supabase is live, sign-up sends a confirmation email — show a notice
    if (tab === 'signup' && supabaseReady) {
      setDone(true);
    } else {
      onClose('success');
    }
  ...
  // ── Post sign-up confirmation screen (Supabase email verification) ─────────
  if (done) {
    return (
      <div
      ...
      >
        <div className="w-full max-w-md rounded-3xl overflow-hidden"
          style={{ background: '#0e0e1a', border: '1px solid rgba(0,245,212,0.3)', boxShadow: '0 24px 80px rgba(0,0,0,0.8)' }}>
        ...
        <div className="p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {tab === 'signup' && (
              <div>
                ...
                <input type="text" value={alias} onChange={e => setAlias(e.target.value)}
                  placeholder="e.g. SolarDrift, NeonDreamer…" maxLength={30}
                  className="w-full px-4 py-3 rounded-xl text-sm text-white outline-none transition-all"
                  style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', caretColor: '#9b5de5' }}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPw(s => !s)}
                  ...
                  style={{ color: 'rgba(255,255,255,0.3)' }}>{showPw ? '🙈' : '👁'}</button>
              </div>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading
                ? <span className="flex items-center justify-center gap-2"><span className="animate-spin">⏳</span> Please wait…</span>
          ...
          {tab === 'signup' && (
            <p className="text-center text-xs mt-4" style={{ color: 'rgba(255,255,255,0.25)' }}>
              Your email is only for account recovery — never shown to other users.
            </p>
          )}

          <button onClick={() => onClose('cancel')}
            className="w-full pt-4 text-xs text-center block"
